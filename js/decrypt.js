...
if (process.argv.length >= 5) {
    ...
    var cipherTextBuf = new Buffer(process.argv[2], 'base64');
    var secretKeyBuf = new Buffer(process.argv[3], 'base64');
    var nonceBuf = new Buffer(process.argv[4], 'base64');
    ...
    if (!sodium.crypto_secretbox_open_easy(plainTextBuf, cipherTextBuf, nonceBuf, secretKeyBuf)) {
        ...
        console.log('Decrypted message:', plainTextBuf, '(' + plainTextBuf.toString() + ')')
    }

} else {
    log.error("Pass in a 'encryptedMessage', 'secretKey', and 'nonce")
}